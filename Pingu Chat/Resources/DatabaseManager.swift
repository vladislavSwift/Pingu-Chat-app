...
extension DatabaseManager {
    ...
    public func getDataFor(path: String, completion: @escaping (Result<Any, Error>) -> Void) {
        ...
        database.child("\(path)").observeSingleEvent(of: .value) { snapshot in      //should have with:
...
extension DatabaseManager {
    ...
    /// Parameters
    /// - `email`:                  Target email to check if it exists
...
extension DatabaseManager {
    ...
    /// Creates a new conversation with target user email and message sent
        ...
        ref.observeSingleEvent(of: .value, with: { [weak self] snapshot in
            ...
            self?.database.child("\(otherUserEmail)/conversations").observeSingleEvent(of: .value, with: { [weak self] snapshot in
                ...
                if var conversations = snapshot.value as? [[String: Any]] {
                    //If conversation exists append data
                    
                    conversations.append(recipient_newConversationData)
                    self?.database.child("\(otherUserEmail)/conversations").setValue(conversations)
                    
                } else {
                    //conversation doesnt exist so create
    ...
    ///Gets and display all messages for a conversation
        ...
        database.child("\(id)/messages").observe(.value, with: { snapshot in
            ...
            let messages: [Message] = value.compactMap({ dictionary in
                ...
                guard let name = dictionary["name"] as? String,
                    ...
                    let messagedID = dictionary["id"] as? String,
            ...
            strongSelf.database.child("\(conversation)/messages").setValue(currentMessages) { error, _ in
                ...
                strongSelf.database.child("\(currentEmail)/conversations").observeSingleEvent(of: .value, with: { snapshot in
                    ...
                    strongSelf.database.child("\(currentEmail)/conversations").setValue(databaseEntryConversations, withCompletionBlock: { error, _ in
                        ...
                        //Update the latest message for recipient user
            ...
            if var conversations = snapshot.value as? [[String: Any]]  {
                ...
                ref.setValue(conversations, withCompletionBlock: { error, _ in
                    ...
                    guard error == nil else {
                        ...
                        print("eroor occured while deleting conversation \(error ?? "eroor occured read console" as! Error)")
                        return
                    }
                    
                    print("deleted conversation successful")
                    completion(true)
